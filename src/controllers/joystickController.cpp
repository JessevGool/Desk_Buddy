...
namespace DeskBuddy
{
    ...
    JoystickDirection JoystickController::getCurrentDirection(int horiz, int vert)
    {
        ...
        if (abs(horizDelta) >= abs(vertDelta)) {
            if (horizDelta >= DIRECTION_OFFSET) return JoystickDirection::RIGHT;
            if (horizDelta <= -DIRECTION_OFFSET) return JoystickDirection::LEFT;
        } else {
            if (vertDelta >= DIRECTION_OFFSET) return JoystickDirection::DOWN;
            if (vertDelta <= -DIRECTION_OFFSET) return JoystickDirection::UP;
        }
    ...
    bool JoystickController::isSelectPressed(int selectValue)
    {
